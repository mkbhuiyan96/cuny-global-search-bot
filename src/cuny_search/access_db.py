...
async def get_or_create_uid(conn: Connection, course_params: CourseParams) -> tuple[int, bool]:
    try:
        async with conn.cursor() as cursor:
...
async def get_course_details(conn: Connection, uid: int) -> Row | None:
    try:
        async with conn.cursor() as cursor:
            await cursor.execute("""
                SELECT *
                FROM course_params
                JOIN course_details ON course_params.uid = course_details.uid
...
async def get_course_availability(conn: Connection, uid: int) -> Row | None:
    try:
        async with conn.cursor() as cursor:
            await cursor.execute("""
                SELECT *
                FROM course_params
                JOIN course_availabilities ON course_params.uid = course_availabilities.uid
                WHERE course_availabilities.uid = ?
            """, (uid,))
            return await cursor.fetchone()
    except Exception as e:
        ic(f"Error occurred while trying to get course and details for {uid}: {e}")
        return None
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            await cursor.execute("""
                SELECT uid
                FROM course_params
                WHERE course_base64 = ? AND session = ? AND term_code = ?
            """, course_params.get_encoded_tuple())
            row = await cursor.fetchone()

            if len(rows) > 1 and not row:
                return AMBIGUOUS
            if not row:
                return NOT_FOUND
            uid = row[0]
        ...
        ic(f"Error while removing interest for uid={uid}, user_id={user_id}: {e}")
        return NOT_FOUND
